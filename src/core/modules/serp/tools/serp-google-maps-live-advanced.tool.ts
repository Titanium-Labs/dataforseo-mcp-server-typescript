...
export class SerpGoogleMapsLiveAdvancedTool extends BaseTool {
  ...
  getDescription(): string {
    return 'Get Google Maps live results for a place or business query.';
  }

  getParams(): z.ZodRawShape {
    return {
      keyword: z.string().describe('Search term or place name'),
...
Location format - hierarchical, comma-separated (from most specific to least)
 ...
 3. City,Region,Country: "San Francisco,California,United States"`),
      ...
      latitude: z.number().optional().describe('Latitude for map centering'),
      longitude: z.number().optional().describe('Longitude for map centering'),
